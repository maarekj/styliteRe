...
type mediaQuery = (option(string), list(rule));
...
let print_selectors = selectors => Utils.join(", ", selectors);

let print_rule = rule =>
  switch rule {
  | (selectors, declarations) => print_selectors(selectors) ++ " {\n" ++ print_declarations(declarations) ++ "}"
  };

let print_rules = (rules: list(rule)) => Utils.join("\n", List.map(print_rule, rules));

let print_media_query = mediaQuery =>
  switch mediaQuery {
  | (None, rules) => print_rules(rules)
  | (Some(mediaQuery), rules) => Printf.sprintf("@media %s {\n%s\n}", mediaQuery, print_rules(rules))
  };

let print_media_queries = (mediaQueries: list(mediaQuery)) =>
  Utils.join("\n", List.map(print_media_query, mediaQueries));