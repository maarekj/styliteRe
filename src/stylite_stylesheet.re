...
let get_all_rules stylesheet => {
  let rules =
    StringMap.mapi
      (fun classname rules => replace_amp_by_classname ("." ^ classname) rules) stylesheet.map;
  let rules = List.map (fun (_, value) => value) (StringMap.bindings rules);
  List.flatten rules
};

let remove_listener stylesheet id =>
  stylesheet.listeners = stylesheet.listeners |> List.filter (fun item => item.id != id);
...
let add_listener stylesheet listener => {
  ...
  fun () => remove_listener stylesheet id
};
